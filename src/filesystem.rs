...
impl InstructionReader for FileSystem {
    ...
    fn write_service_hierarchy(&mut self, hierarchy: &ServiceHierarchy) {
        let filename = format!("{}.json", hierarchy.service.to_ascii_lowercase());
        let mut file = File::create(self.root.join(&filename))
            .unwrap_or_else(|error| panic!("can't create {}: {:?}", filename, error));
        file.write_all(
            &serde_json::to_vec_pretty(hierarchy).expect("couldn't serialize service hierarchy"),
        )
        .unwrap_or_else(|error| panic!("can't write {}: {:?}", filename, error));
    }
